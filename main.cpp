...
GLint   loc_a_colorFactor;
GLint   loc_a_metalFactor;
GLint   loc_a_roughFactor;
...
void init_shader_program()
{
  ...
  if(filename == "Box.gltf") fragment_shader = create_shader_from_file("./shader/fragment1.glsl", GL_FRAGMENT_SHADER);
  ...
  loc_u_base_color = glGetUniformLocation(program, "u_base_color"); 
  loc_u_is_base = glGetUniformLocation(program, "u_is_base");
...
void init_texture_objects()
{
  ...
  for (const tinygltf::Texture& texture : textures)
  {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA,
      image.width, image.height, 0, format, type, &image.image[i]);
	i++;
  ...
  for (const tinygltf::Primitive& primitive : mesh.primitives)
  {
    if (primitive.material > -1)
    {
      ...
      for (const std::pair<std::string, tinygltf::Parameter> parameter : material.values)
      {
		...
		{
		  std::vector<double> v = parameter.second.number_array;
		  
		  glUniform4fv(loc_u_base_color, 1, kmuvcl::math::vec4f(
		  v[0],v[1],v[2],v[3]));
...
{
  if (argc > 2) {
	std::cout << "need Argument only one" << std::endl;
	return 0;
  }
